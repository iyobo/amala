...
### [v13.1.0](https://github.com/iyobo/amala/compare/v13.0.1...v13.1.0)

> 16 September 2026

- Accept any Standard Schema validator directly in `@Body`, `@Query`, and `@Params`, for complete request values or selected properties.
- Inject validated output so schemas can safely coerce, transform, default, and validate asynchronously before controller code runs.
- Normalize validation issues into safe `422` details without returning rejected input or validator internals; convert unexpected validator failures into generic `500` responses.
- Generate request-body and parameter documentation from the optional Standard JSON Schema `openapi-3.0` capability, while omitting unsupported schema details instead of inventing them.
- Preserve the existing class-validator path without requiring an adapter, registry, flag, or migration.
- Add Zod and Valibot integration coverage, OpenAPI capability coverage, and a matched Amala/Koa/Fastify Standard Schema benchmark workload.
- Update the README and public documentation with complete controller-and-bootstrap examples for validator-neutral request handling.

#### [v13.0.1](https://github.com/iyobo/amala/compare/v13.0.0...v13.0.1)

> 15 September 2026

- Preserve present falsy request bodies such as `false`, `0`, and `""` when an injected body is marked as required.
- Keep endpoint-specific deprecation headers isolated to the endpoint that declared them.
- Correct generated OpenAPI 3.0 documents by using the uppercase `2XX` response range, marking path parameters as required, and documenting JSON request bodies.
- Add regression coverage for each corrected behavior.

### [v13.0.0](https://github.com/iyobo/amala/compare/v12.0.0...v13.0.0)

> 3 September 2026

- **Breaking:** Attach generated router middleware to the Koa application by default so `bootstrapControllers()` returns an app that is ready to listen.
- Preserve advanced middleware ordering through explicit `attachRoutes: false` and continue returning the generated router for manual composition.
- Add regression coverage for automatic attachment, explicit opt-out, and manual router mounting.
- Replace the README with a progressive, example-driven guide covering Hello World, request arguments, validation, typed request context, authentication middleware, versioning, OpenAPI, uploads, composition, errors, and production configuration.
- Preserve the v12 and v11 documentation, add a v13 migration guide, and update the public release experience for v13.
- Keep the legacy TypeScript decorator implementation unchanged; migration to standard decorators remains a separate API design decision.

### [v12.0.0](https://github.com/iyobo/amala/compare/v11.0.1...v12.0.0)

> 3 September 2026

- **Breaking:** Preserve user-defined Koa state and context-extension generics through bootstrap, middleware, controller construction, error handling, body-parser errors, and returned app/router types.
- **Breaking:** Replace Koa's open-ended default state and context types with safe property-free defaults. Applications that depend on global Koa augmentation can opt into `Koa.DefaultState` and `Koa.DefaultContext` explicitly.
- **Breaking:** Type custom error-handler errors as `unknown` and replace decorator metadata's unchecked values with `unknown` where runtime narrowing is required.
- Add exported `AmalaContext`, `AmalaMiddleware`, `AmalaNext`, and `EmptyContext` types.
- Ship the Koa and CORS type packages needed by Amala's public declarations as package dependencies so they resolve after a normal install.
- Enable `noImplicitAny` for the package and add compile-time regression coverage for declared and undeclared context properties.
- Prevent the default error handler from writing raw server exceptions to logs; it now records only the response status unless an application supplies a redacting custom handler.
- Correct generated OpenAPI request schemas so required body fields use the schema-level `required` array and unrecognized class names fall back to the `object` type.
- Preserve the v11 documentation, add a documentation version selector and v12 migration guide, and refresh the public landing page for the v12 release.
- Keep the legacy TypeScript decorator implementation in v12; migration to standard decorators remains a separate API design decision.

#### [v11.0.1](https://github.com/iyobo/amala/compare/v11.0.0...v11.0.1)

> 3 September 2026

- Remove redundant direct `@types/cookies` and `@types/express` development dependencies. Koa supplies its required cookie types, and Amala does not expose an Express API.

### [v11.0.0](https://github.com/iyobo/amala/compare/v10.1.0...v11.0.0)

> 3 September 2026

- Require Node.js 22 or newer.
- Upgrade Koa to 3.2, `@koa/router` to 15.7, and `koa-body` to 8.0.
- Upgrade the re-exported `class-validator` API to 0.15.1.
- Remove the unused `ts-morph` development dependency.
- **Breaking:** Koa 3, Router 15, and koa-body 8 include upstream behavior and type changes described in the v11 migration guide.
- **Breaking:** `@IsIBAN()` now accepts IBAN options first and validation options second. Existing validation-message arguments must move to the second position.
- Add a v11 migration guide and regression coverage for the new decorator signature and upgraded runtime stack.

### [v10.1.0](https://github.com/iyobo/amala/compare/v10.0.9...v10.1.0)

> 3 September 2026

- Add a request-aware `controllerFactory` option for container-neutral dependency injection.
- Preserve the existing per-request `new ControllerClass(ctx)` behavior when no factory is configured.

#### [v10.0.9](https://github.com/iyobo/amala/compare/v10.0.8...v10.0.9)

> 3 September 2026

- Fix generated OpenAPI URLs so `basePath` is included exactly once and generated state does not leak between applications.
- Preserve nested class-validator failures as safe, dot-separated error details.
- Support single and multiple uploads from `@koa/multer` through `@File()` while retaining koa-body compatibility.

#### [v10.0.8](https://github.com/iyobo/amala/compare/v10.0.8...v10.0.8)

#### [v10.0.8](https://github.com/iyobo/amala/compare/v10.0.7...v10.0.8)

> 9 January 2024

- Release [`353cb37`](https://github.com/iyobo/amala/commit/353cb378cdfaace143d4c1f94507782eae0f9267)
- update docs [`060eaf4`](https://github.com/iyobo/amala/commit/060eaf47ab84b8ff6b69e05fe86a42ecbac0b524)

#### [v10.0.7](https://github.com/iyobo/amala/compare/v10.0.6...v10.0.7)

> 9 January 2024

- docs init [`f76c8ad`](https://github.com/iyobo/amala/commit/f76c8adad5e27efe700444d230a5e3aac31e1a2f)
- docs [`b6d8d28`](https://github.com/iyobo/amala/commit/b6d8d289954571b529d681095b29843b51d417b4)
- docs finished [`6a29104`](https://github.com/iyobo/amala/commit/6a29104d7342ffb19cff71ade93842b891f537e0)

#### [v10.0.6](https://github.com/iyobo/amala/compare/v10.0.5...v10.0.6)

> 2 January 2024

- audit patch [`d930d79`](https://github.com/iyobo/amala/commit/d930d79844fe97bcb09b7aa08690d6e1989e7b2b)
- Release [`a0040ce`](https://github.com/iyobo/amala/commit/a0040ce9296cc32da2e9ce27aea4f876be3ebecf)
...
- Release [`315f1b4`](https://github.com/iyobo/amala/commit/315f1b4605a7bd3663956d1f441e1ba8860d2bc3)
...
#### [v9.1.1](https://github.com/iyobo/amala/compare/v9.1.0...v9.1.1)
